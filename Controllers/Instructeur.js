        ...
        Instructeur.findOne({
          ...
          .then((instructeur) => {
            if (!instructeur) {
                    ...
                    html: `
                     <div>
                      <h1>Email d'activation du compte </h1>
                        <h2>Bonjour </h2>
                      <p>ravi détre avec nous 
                      <a href=http://localhost:3000/activationpage/${instructeurData.activationCode}>Cliquez ici</a>                              
                       </div>`,
              ...
              res.status(409).send({ message: "Formateur already exist" });
            }
          })
          .catch((err) => {
            res.send("error: " + err);
